...
# 클래스 선언
class Test:              # 클래스명은 첫글자 대문자(권장)
    str = 'This is Class'      # str : Test 클래스의 요소(attribute - field or member)

# 클래스 호출
test1 = Test()       # test1은 Test 클래스에 의해서 만들어진 인스턴스
print(test1.str)
...
그 중 init()은 클래스가 인스턴스화 할 때 호출되는 메소드
 - 초기화 메소드
'''
...
# 클래스 변수 특성
 - 클래스로부터 생성된 모든 인스턴스들이 접근할 수 있다 (공유된다)
 - 어떤 객체가 클래스 변수를 변경한다면 다른 인스턴스들에게도 변경내용이 반영
 
# 객체(인스턴스) 변수
 - 클래스로부터 생성된 각각의 객체(인스턴스)에 속해 있는 변수
...
class Character:
    ...
    def attack(self):
        ...
        if Character.cnt == 0:
            print('더이상 캐릭터가 남아있지 않습니다.')
            return
        else :
            print('아직 {} 명의 캐릭터가 남아있습니다'.format(Character.cnt))

    @classmethod
    def charaterInfo(self):
        print('현재 {} 개의 캐릭터가 있습니다.'.format(Character.cnt))

gameCharater1 = Character('마린1')
gameCharater1.say()

gameCharater2 = Character('마린2')
gameCharater2.say()

print('----------------------------------')
Character.charaterInfo()

print('------------------------------------')
gameCharater3 = Character('마린3')

Character.attack(gameCharater2)
